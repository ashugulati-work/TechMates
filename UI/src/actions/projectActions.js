...
export const deleteMessageValue = (deleteId) => async (dispatch) => {
   ...
   var getDeletedResponse = await fetch(
      constant.SERVER_HOST_AND_PORT + `/api/sentence/${deleteId}`,
      requestOptions
   )
...
export const submitFormData =
   (messages, rowInput, dropDownValue, tone, keywords) => async (dispatch) => {
      const requestOptions = {
         method: 'POST',
         headers: {'Content-Type': 'application/json'},
         body: JSON.stringify({
            no_of_sentences: rowInput,
            sentences: messages,
            topic: dropDownValue,
            tone: tone,
            keywords: keywords
      ...
      var getAllMessagesResponse = await fetch(
         constant.SERVER_HOST_AND_PORT + '/api/sentences',
         requestOptions
      )
      ...
      var getDeletedResponse = await fetch(
         constant.SERVER_HOST_AND_PORT + `/api/sentence`,
         requestOptionsEmpty
      )

      if (
         getAllMessagesResponse &&
         getAllMessagesResponse.status === 200 &&
         getDeletedResponse &&
         getDeletedResponse.status === 200
      ) {
         getAllMessagesResponse = await getAllMessagesResponse.json()
         dispatch({
            type: RETRIVE_MSG_SUCCESS,
            payload: {getInputMsg: [], retriveMsg: getAllMessagesResponse.data}
         })
      } else {
...
export const downloadFile = () => async (dispatch) => {
   ...
   var getDownloadResponse = await fetch(
      constant.SERVER_HOST_AND_PORT + '/api/sentence_file',
      requestOptions
   )
