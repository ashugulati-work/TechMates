...
const InputMessages = ({ messages, deleteMessageValue }) => {
    ...
    const deleteMessage = (index) => {
        console.log(index); // Print the index to the console
        deleteMessageValue(index);
    }

    return (
        <> {Object.keys(msgData).length === 0 ? '' :  <ul className="list-group"> {
            msgData.map((value, index) => {
                return (<li key={index} className="list-group-item list-group-item-action li-font-size"><span onClick={() => deleteMessage(index)} className='delete-icon'>
